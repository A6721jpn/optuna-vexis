...
Supports lightweight feasibility checking (--check-only mode) for
FreeCAD-First feasibility manifold learning.
"""
...
class GeometryAdapter:
    ...
    def _worker_script_path(self) -> Path:
        return self._project_root / "src" / "v2-claude" / "freecad_worker.py"
    ...
    def check_feasibility(self, params: dict[str, float]) -> dict:
        """Lightweight FreeCAD feasibility check (no STEP export).

        Runs the worker in --check-only mode: apply ratios, recompute,
        and validate surface geometry.  ~0.5s per call.

        Args:
            params: Dictionary of {constraint_name: ratio_value}.

        Returns:
            {"feasible": bool} parsed from worker JSON output.
            On worker failure, returns {"feasible": False, "error": str}.
        """
        worker = self._worker_script_path()
        if not worker.exists():
            raise GeometryError(f"FreeCAD worker script not found: {worker}")
        ...
        with tempfile.TemporaryDirectory(prefix="v2c_check_") as tmpdir:
            tmp = Path(tmpdir)
            constraints_json = tmp / "constraints.json"
            params_json = tmp / "params.json"
            ...
            logger.debug("FreeCAD check-only run: %s", " ".join(cmd))
            try:
                proc = subprocess.run(
                    cmd,
                    cwd=str(self._project_root),
                ...
                return {"feasible": False, "error": f"timeout_after_{timeout_sec}s"}

            if proc.returncode != 0:
                stderr = (proc.stderr or "").strip()
                stdout = (proc.stdout or "").strip()
                detail = stderr or stdout or "no output"
                return {"feasible": False, "error": detail}

            stdout = (proc.stdout or "").strip()
            # Parse last line as JSON (worker may emit logging before JSON)
            for line in reversed(stdout.splitlines()):
                ...
                if line.startswith("{"):
                    try:
                        ...
                        continue
            return {"feasible": False, "error": f"no_json_output: {stdout[:200]}"}
