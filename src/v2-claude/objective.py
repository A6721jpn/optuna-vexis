"""
v2-claude Objective Orchestrator

Unified objective function that implements the 3-tier decision policy:
  1. Check hard constraints
  2. FeasibilityOracle 3-tier evaluation (surrogate + FreeCAD)
     - high_confidence: skip FreeCAD, proceed to STEP
     - uncertain: verify with FreeCAD lightweight check
     - rejected: penalty (with exploration bypass)
  3. Geometry generation (STEP export)
  4. CAE evaluation
  5. Metric -> objective value(s)

Falls back to legacy CadGate when FeasibilityOracle is not provided.
...
FAILURE_STAGE_ATTR = "v2_failure_stage"
FAILURE_REASON_ATTR = "v2_failure_reason"
...
class ObjectiveOrchestrator:
    """Wire together all v2-claude components into one callable objective.
    ...
    When a ``FeasibilityOracle`` is provided, uses 3-tier evaluation
    (surrogate + FreeCAD lightweight check). Otherwise falls back to
    the legacy ``CadGate`` binary prediction.
    """
    ...
    def _evaluate(
        ...
        *,
        ...
        # 2. Feasibility evaluation (3-tier oracle or legacy CadGate)
        #    In dry_run mode, skip FreeCAD subprocess calls and use
        #    surrogate-only prediction (no ground-truth verification).
        if self._oracle is not None and not dry_run:
            is_feasible, surrogate_score, tier = self._oracle.evaluate(
                point.params, trial_id=point.trial_id,
            )
            record.feasibility = CadFeasibilityResult(
                is_feasible=is_feasible,
                confidence=surrogate_score,
                reason_code=None if is_feasible else f"oracle_{tier}",
                metadata={"tier": tier, "surrogate_score": surrogate_score},
            )
            if not is_feasible:
                violation = 1.0 - surrogate_score
                logger.info(
                    "Trial %d: infeasible (tier=%s, score=%.3f)",
                    point.trial_id, tier, surrogate_score,
                )
                record.outcome = TrialOutcome.CAD_INFEASIBLE
                self._store_feasibility(trial, violation)
                self._store_failure_context(
                    trial,
                    stage=f"oracle_{tier}",
                    reason=f"surrogate_score={surrogate_score:.4f}",
                )
                return self._penalty(point, TrialOutcome.CAD_INFEASIBLE)
            logger.debug(
                "Trial %d: feasible (tier=%s, score=%.3f)",
                point.trial_id, tier, surrogate_score,
            )
        elif self._oracle is not None and dry_run:
            # Dry run: surrogate-only prediction, no FreeCAD calls
            surrogate_score = self._oracle.predict_surrogate(point.params)
            record.feasibility = CadFeasibilityResult(
                is_feasible=True,
                confidence=surrogate_score,
                reason_code=None,
                metadata={"tier": "dry_run", "surrogate_score": surrogate_score},
            )
            logger.debug(
                "Trial %d: dry_run feasibility (surrogate_score=%.3f)",
                point.trial_id, surrogate_score,
            )
        else:
            # Legacy CadGate path
