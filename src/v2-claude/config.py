"""
v2-claude Configuration

Load, validate, and expose typed configuration from YAML files.

v2-claude extends v2 with FreeCAD-First feasibility manifold learning:
  - Phase 0 feasibility discovery using FreeCAD lightweight checks
  - Interaction-aware surrogate with pairwise feature engineering
  - 3-tier feasibility judgment (high/uncertain/reject)
...
class FeasibilitySpec:
    """FreeCAD-First feasibility manifold learning settings."""

    # Phase 0: Feasibility discovery
    discovery_points: int = 300
    discovery_bounds_margin: float = 0.15  # ±15% sampling (wider than config bounds)

    # Surrogate model
    surrogate_retrain_interval: int = 20  # retrain every N trials
    interaction_features: bool = True  # include all pairwise interaction terms (210 dims)

    # 3-tier thresholds for surrogate score
    tier_high_confidence: float = 0.7  # score >= this → skip FreeCAD check
    tier_uncertain: float = 0.2  # score in [this, high_confidence) → FreeCAD lightweight check
    # score < tier_uncertain → reject (penalty), except exploration bypass

    # Exploration bypass
    exploration_ratio: float = 0.05  # fraction of rejected trials that bypass to FreeCAD anyway

    # Feasibility DB
    db_path: Optional[str] = None  # path to persist feasibility data; None = in-memory only
...
@dataclass
class V2ClaudeConfig:
    """Aggregated configuration for a v2-claude run."""
...
def load_config(
    ...
    """Load and validate both YAML files into a single v2-claude config object."""
    ...
    # --- Feasibility (v2-claude specific) ---
    feas_raw = raw_lim.get("feasibility", {})
    feasibility = FeasibilitySpec(
        discovery_points=int(feas_raw.get("discovery_points", 300)),
        discovery_bounds_margin=float(feas_raw.get("discovery_bounds_margin", 0.15)),
        surrogate_retrain_interval=int(feas_raw.get("surrogate_retrain_interval", 20)),
        interaction_features=bool(feas_raw.get("interaction_features", True)),
        tier_high_confidence=float(feas_raw.get("tier_high_confidence", 0.7)),
        tier_uncertain=float(feas_raw.get("tier_uncertain", 0.2)),
        exploration_ratio=float(feas_raw.get("exploration_ratio", 0.05)),
        db_path=feas_raw.get("db_path"),
    )
    ...
    _validate(cfg)
    logger.info("v2-claude config loaded: %d design variables", len(cfg.bounds))
    return cfg


def _validate(cfg: V2ClaudeConfig) -> None:
    ...
    if feas.discovery_points < 0:
        raise ValueError("feasibility.discovery_points must be >= 0")
    if not (0.0 < feas.discovery_bounds_margin <= 1.0):
        raise ValueError("feasibility.discovery_bounds_margin must be in (0, 1]")
    if feas.surrogate_retrain_interval < 1:
        raise ValueError("feasibility.surrogate_retrain_interval must be >= 1")
    if not (0.0 <= feas.tier_uncertain <= feas.tier_high_confidence <= 1.0):
        raise ValueError(
            "feasibility tiers must satisfy 0 <= tier_uncertain <= tier_high_confidence <= 1"
        )
    if not (0.0 <= feas.exploration_ratio <= 1.0):
        raise ValueError("feasibility.exploration_ratio must be in [0, 1]")
...
V2Config = V2ClaudeConfig
V1Config = V2ClaudeConfig
