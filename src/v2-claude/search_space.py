...
FEASIBILITY_ATTR = "v2_feasibility_violation"
...
def make_constraints_func() -> Callable[[FrozenTrial], Sequence[float]]:
    ...
    Convention:
      - ``trial.user_attrs["v2_feasibility_violation"]`` is a float
      - <= 0  → feasible
...
class FeasibilityAwareSampler(BaseSampler):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            threshold_hint: CAD gate threshold used to interpret score values.
            exploration_enabled: Enable mixed exploration policies.
            global_ratio: Weight for random global exploration.
            boundary_ratio: Weight for boundary-focused exploration.
            local_ratio: Weight for local refinement near feasible archive.
            boundary_candidate_pool: Candidate count for boundary selection.
            uncertainty_band: |score-threshold| band for uncertain pass-through.
            uncertainty_accept_prob: Acceptance probability in uncertainty band.
            local_perturbation_scale: Local perturbation span ratio.
            local_archive_size: Maximum stored feasible candidates for local mode.
            random_seed: Seed for sampler-side strategy RNG.
        """
        ...
        self._threshold_hint = float(threshold_hint)
        self._exploration_enabled = bool(exploration_enabled)
        self._global_ratio = max(0.0, float(global_ratio))
        self._boundary_ratio = max(0.0, float(boundary_ratio))
        self._local_ratio = max(0.0, float(local_ratio))
        self._boundary_candidate_pool = max(1, int(boundary_candidate_pool))
        self._uncertainty_band = max(0.0, float(uncertainty_band))
        self._uncertainty_accept_prob = min(max(float(uncertainty_accept_prob), 0.0), 1.0)
        self._local_perturbation_scale = max(1e-6, float(local_perturbation_scale))
        self._local_archive_size = max(1, int(local_archive_size))
        self._rng = random.Random(random_seed)
        self._stats_accepted = 0
        self._stats_rejected = 0
        self._stats_repaired = 0
        self._stats_uncertain_accepted = 0
        ...
        self._feasible_archive: list[dict[str, float]] = []
    ...
    def _choose_strategy(self) -> str:
        if not self._exploration_enabled:
            return "base"
        total = self._global_ratio + self._boundary_ratio + self._local_ratio
        if total <= 0.0:
            return "base"
        pick = self._rng.random() * total
        if pick < self._global_ratio:
            return "global"
        pick -= self._global_ratio
        if pick < self._boundary_ratio:
            return "boundary"
        return "local"

    def _register_feasible_candidate(self, params: dict[str, float]) -> None:
        self._feasible_archive.append(dict(params))
        if len(self._feasible_archive) > self._local_archive_size:
            self._feasible_archive.pop(0)

    def _sample_boundary_candidate(
        self,
        study: Study,
        trial: FrozenTrial,
        ...
        if not search_space:
            return {}
        if self._predict_score_fn is None:
            return self._sample_random_candidate(study, trial, search_space)

        best: dict[str, float] | None = None
        best_dist = float("inf")
        for _ in range(self._boundary_candidate_pool):
            candidate = self._sample_random_candidate(study, trial, search_space)
            candidate = self._align_params_to_space(candidate, search_space)
            full_candidate = self._inject_fixed_params(candidate)
            try:
                score = float(self._predict_score_fn(full_candidate))
            except Exception:
                continue
            dist = abs(score - self._threshold_hint)
            if dist < best_dist:
                best_dist = dist
                best = dict(candidate)
    ...
    def _sample_local_candidate(
        ...
        if not search_space:
            return {}
        if not self._feasible_archive:
            return self._sample_full_candidate(study, trial, search_space)

        anchor = self._rng.choice(self._feasible_archive)
        candidate: dict[str, float] = {}
        for name, dist in search_space.items():
            if not isinstance(dist, FloatDistribution):
                candidate[name] = float(
                    self._safe_base_sample_independent(study, trial, name, dist)
                )
                continue
            ...
            hi = float(dist.high)
            span = max(0.0, hi - lo)
            base = float(anchor.get(name, (lo + hi) / 2.0))
            if dist.step is None:
                delta = span * self._local_perturbation_scale
                v = base + self._rng.uniform(-delta, delta)
            else:
                step = float(dist.step)
                if step <= 0.0:
                    v = base
                else:
                    total_steps = max(0, int(round((hi - lo) / step)))
                    anchor_idx = int(round((base - lo) / step))
                    window = max(
                        1,
                        int(round(total_steps * self._local_perturbation_scale)),
                    )
                    min_idx = max(0, anchor_idx - window)
                    max_idx = min(total_steps, anchor_idx + window)
                    idx = self._rng.randint(min_idx, max_idx)
                    v = lo + (idx * step)
            if v < lo:
                v = lo
            elif v > hi:
                v = hi
            candidate[name] = float(v)

        return candidate

    def _sample_candidate_by_strategy(
        self,
        study: Study,
        trial: FrozenTrial,
        search_space: dict[str, BaseDistribution],
        ...
        if strategy == "boundary":
            return self._sample_boundary_candidate(study, trial, search_space)
        if strategy == "local":
            return self._sample_local_candidate(study, trial, search_space)
        return self._sample_full_candidate(study, trial, search_space)

    def _maybe_accept_uncertain(self, score: float) -> bool:
        if self._uncertainty_accept_prob <= 0.0:
            return False
        if abs(score - self._threshold_hint) > self._uncertainty_band:
            return False
        return self._rng.random() < self._uncertainty_accept_prob
    ...
    def sample_relative(
        ...
        if not search_space and self._fixed_params:
            ...
            if self._predict_score_fn is not None:
                ...
                feasible = score >= self._threshold_hint
            else:
                feasible = self._predict_fn(params)
                score = 1.0 if feasible else 0.0
            if feasible:
                self._stats_accepted += 1
                self._register_feasible_candidate(params)
                return {}
            if self._maybe_accept_uncertain(score):
                self._stats_accepted += 1
                self._stats_uncertain_accepted += 1
                return {}
        ...
        for _ in range(retries):
            sampled = self._sample_candidate_by_strategy(study, trial, search_space)
        ...
        if self._repair_fn is not None:
            ...
            if repaired is not None:
                if self._predict_score_fn is not None:
                    repaired_score = self._score(repaired)
                    repaired_feasible = repaired_score >= self._threshold_hint
                else:
                    repaired_feasible = self._predict_fn(repaired)
                if repaired_feasible:
                    self._stats_accepted += 1
                    self._stats_repaired += 1
                    self._register_feasible_candidate(repaired)
                    repaired_relative = {
                        name: repaired[name]
    ...
    def rejection_stats(self) -> dict[str, int]:
        return {
            ...
            "uncertain_accepted": self._stats_uncertain_accepted,
        }
