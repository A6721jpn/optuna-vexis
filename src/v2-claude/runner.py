...
Key features:
  - Phase 0 feasibility discovery: map the feasible manifold using
    FreeCAD lightweight checks before optimization starts.
  - FeasibilityOracle: interaction-aware surrogate replaces static ML gate.
  - 3-tier feasibility judgment in objective (high/uncertain/reject).
  - ``constraints_func`` injected into TPE/NSGA-II so the sampler
    learns the CAD feasibility boundary from trial history.
  - ``FeasibilityAwareSampler`` uses oracle surrogate for rejection sampling.
...
def _setup_logging(log_dir: Path, level: str = "INFO") -> None:
    ...
    log_file = log_dir / f"v2_{timestamp}.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Production v2-claude: FreeCAD-First feasibility manifold learning optimization with VEXIS"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limits", "-l", default="config/v2_claude_limitations.yaml",
        help="v2-claude limitations YAML",
    )
    parser.add_argument("--max-trials", "-n", type=int, default=None)
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--skip-discovery", action="store_true",
                        help="Skip Phase 0 feasibility discovery")
...
def main() -> int:
    ...
    logger.info("v2-claude optimization start")
    ...
    try:
        ...
        storage_path = result_dir / "optuna_study_v2.db"
        ...
        # --- FeasibilityOracle (v2-claude core) ---
        ...
        if not args.skip_discovery and cfg.feasibility.discovery_points > 0:
            logger.info("Phase 0: starting feasibility discovery")
            discovery_summary = oracle.run_discovery()
            persistence.save_run_config({
                "phase0_discovery": discovery_summary,
            })
            logger.info(
                "Phase 0 complete: %d/%d feasible (%.1f%%)",
                discovery_summary["feasible"],
                discovery_summary["n_points"],
                discovery_summary["feasible_rate"] * 100,
            )
        else:
            if args.skip_discovery:
                logger.info("Phase 0 discovery skipped (--skip-discovery)")
            else:
                logger.info("Phase 0 discovery skipped (discovery_points=0)")
        ...
        persistence.save_run_config({
            ...
            "feasibility": {
                "discovery_points": cfg.feasibility.discovery_points,
                "discovery_bounds_margin": cfg.feasibility.discovery_bounds_margin,
                "surrogate_retrain_interval": cfg.feasibility.surrogate_retrain_interval,
                "interaction_features": cfg.feasibility.interaction_features,
                "tier_high_confidence": cfg.feasibility.tier_high_confidence,
                "tier_uncertain": cfg.feasibility.tier_uncertain,
                "exploration_ratio": cfg.feasibility.exploration_ratio,
            },
            "exploration": {
                "enabled": cfg.cad_gate.exploration.enabled,
        ...
        #    level using oracle surrogate predictions.
        #    Falls back to legacy CadGate if oracle surrogate is not trained.
        def _predict_fn(params: dict[str, float]) -> bool:
            """Predict feasibility using oracle surrogate or legacy CadGate."""
            if oracle.surrogate.is_trained:
                score = oracle.predict_surrogate(params)
                return score >= cfg.feasibility.tier_uncertain
            # Fallback to legacy CadGate
        ...
        def _predict_score_fn(params: dict[str, float]) -> float:
            """Return feasibility score from oracle surrogate or legacy CadGate."""
            if oracle.surrogate.is_trained:
                return oracle.predict_surrogate(params)
            if cad_gate._model is not None:
                dummy = DesignPoint(trial_id=-1, params=params)
        ...
        def _repair_fn(params: dict[str, float]) -> dict[str, float] | None:
            ...
            # Use feasible archive from oracle DB for diverse repair
            feasible_records = [
                r for r in oracle.db.records if r.feasible
            ]
            if feasible_records:
                # Find nearest feasible point and interpolate
                import numpy as np
                param_names = list(baseline_params.keys())
                cand_vec = np.array([candidate.get(n, 1.0) for n in param_names])
                best_anchor = None
                best_dist = float("inf")
                for rec in feasible_records[-50:]:  # last 50 for efficiency
                    fvec = np.array([rec.params.get(n, 1.0) for n in param_names])
                    d = float(np.linalg.norm(cand_vec - fvec))
                    if d < best_dist:
                        best_dist = d
                        best_anchor = rec.params
                if best_anchor is not None:
                    # Binary search from feasible anchor toward candidate
                    best = dict(best_anchor)
                    lo, hi = 0.0, 1.0
                    for _ in range(14):
                        mid = (lo + hi) / 2.0
                        mixed = {
                            name: best_anchor.get(name, 1.0) + mid * (candidate.get(name, 1.0) - best_anchor.get(name, 1.0))
                        ...
                        if _predict_fn(mixed):
                            ...
                            hi = mid
                    return _clip_params(best)

            # Fallback: interpolate from baseline
            baseline_feasible = _predict_fn(baseline_params)
        ...
        fixed_dims_count = sum(
            1
            for dist in fixed_search_space.values()
            if hasattr(dist, "low")
            and hasattr(dist, "high")
            and math.isclose(
                float(dist.low),
                float(dist.high),
                rel_tol=0.0,
        ...
        sampler = FeasibilityAwareSampler(
            ...
            threshold_hint=cfg.feasibility.tier_high_confidence,
        ...
        logger.info(
            "FeasibilityAwareSampler enabled (oracle=%s, max_retries=%d, fixed_dims=%d)",
            "surrogate_trained" if oracle.surrogate.is_trained else "untrained",
        ...
        if isinstance(sampler, FeasibilityAwareSampler):
            ...
            logger.info(
                "Rejection stats: accepted=%d, rejected=%d, repaired=%d, uncertain_accepted=%d",
                rejection_stats["accepted"],
                rejection_stats["rejected"],
                rejection_stats.get("repaired", 0),
                rejection_stats.get("uncertain_accepted", 0),
            )

        # Oracle stats
        oracle_stats = {
            "feasibility_db_size": len(oracle.db),
            "feasible_count": oracle.db.feasible_count,
            "infeasible_count": oracle.db.infeasible_count,
            "surrogate_trained": oracle.surrogate.is_trained,
            "surrogate_train_count": oracle.surrogate.train_count,
        }
        logger.info(
            "Oracle stats: db=%d (feasible=%d, infeasible=%d), surrogate_trained=%s",
            oracle_stats["feasibility_db_size"],
            oracle_stats["feasible_count"],
            oracle_stats["infeasible_count"],
            oracle_stats["surrogate_trained"],
        )
        ...
        logger.info("v2-claude optimization end — best_value=%s", best_value)
        return 0
