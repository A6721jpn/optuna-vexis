...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Production v2: interaction-aware CAD feasibility-gated optimization with VEXIS"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limits", "-l", default="config/v2_limitations.yaml",
        help="v2 limitations YAML",
    )
...
def main() -> int:
    ...
    logger.info("v2 optimization start")
    ...
    try:
        ...
        if cad_gate._model is not None:
            ...
            logger.info(
                "FeasibilityAwareSampler enabled (max_retries=%d, fixed_dims=%d, strategy=(g=%.2f,b=%.2f,l=%.2f), uncertain=(band=%.3f,p=%.2f))",
                cfg.cad_gate.rejection_max_retries,
                fixed_dims_count,
                cfg.cad_gate.exploration.global_ratio,
                cfg.cad_gate.exploration.boundary_ratio,
                cfg.cad_gate.exploration.local_ratio,
                cfg.cad_gate.exploration.uncertainty_band,
                cfg.cad_gate.exploration.uncertainty_accept_prob,
            )
        else:
        ...
        logger.info("v2 optimization end — best_value=%s", best_value)
        return 0
