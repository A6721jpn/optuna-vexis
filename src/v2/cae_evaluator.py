...
def _parse_rigid_body_data_file(data_path: Path) -> Optional[pd.DataFrame]:
    """Parse FEBio rigid body text data into a curve DataFrame."""
        ...
        if line.startswith("*Time") or line.startswith("Time"):
            ...
            if i + 1 < len(lines):
                ...
                if len(parts) >= 3 and current_time is not None:
                    try:
                        disp_z = float(parts[1])
                        force_z = float(parts[2])
                        rows.append((current_time, disp_z, force_z))
                    except ValueError:
                        pass
                i += 1
        i += 1
    ...
    df = pd.DataFrame(rows, columns=["Time", "Disp_Z", "Force_Z"])
    initial_z = float(df["Disp_Z"].iloc[0])
    out = pd.DataFrame(
        {
            "Stroke": initial_z - pd.to_numeric(df["Disp_Z"], errors="coerce"),
            "Reaction_Force": -1.0 * pd.to_numeric(df["Force_Z"], errors="coerce"),
        }
    ).dropna()
    if out.empty:
        return None
...
class CaeEvaluator:
    ...
    def __init__(
        ...
        self._vexis_temp = self._vexis_root / "temp"
    ...
    def evaluate(self, step_path: Path, point: DesignPoint) -> CaeResult:
        ...
        job_name = f"v2_trial_{point.trial_id}"
    ...
    def _run_subprocess(self, job_name: str) -> Optional[Path]:
        ...
        recovered = self._recover_missing_result_csv(job_name, result_csv)
    ...
    def _recover_missing_result_csv(
        self,
        job_name: str,
        result_csv: Path,
    ) -> Optional[Path]:
        """Try to recover missing result CSV from fallback artifacts."""
        generic_csv = self._vexis_results / "force_displacement.csv"
        if generic_csv.exists():
            try:
                result_csv.parent.mkdir(parents=True, exist_ok=True)
                if result_csv.exists():
                    result_csv.unlink()
                generic_csv.replace(result_csv)
                logger.warning(
                    "Recovered result CSV from fallback file: %s -> %s",
                    generic_csv,
                    result_csv,
                )
                return result_csv
            except OSError as exc:
                logger.warning("Failed to move fallback CSV %s: %s", generic_csv, exc)

        data_candidates = [
            self._vexis_results / f"{job_name}_data.txt",
            self._vexis_temp / "rigid_body_data.txt",
        ]
        for data_file in data_candidates:
            for retry in range(10):
                curve = _parse_rigid_body_data_file(data_file)
                if curve is not None and not curve.empty:
                    try:
                        result_csv.parent.mkdir(parents=True, exist_ok=True)
                        curve.to_csv(result_csv, index=False)
                        if data_file.exists():
                            artifact_data = self._vexis_results / f"{job_name}_data.txt"
                            if data_file.resolve() != artifact_data.resolve():
                                shutil.copy2(data_file, artifact_data)
                        logger.warning(
                            "Recovered result CSV from rigid-body data: %s -> %s",
                            data_file,
                            result_csv,
                        )
                        return result_csv
                        ...
                        logger.warning(
                            "Failed to write recovered CSV from %s: %s",
                            data_file,
                            exc,
                        )
                        break
