...
class ObjectiveOrchestrator:
    ...
    def __init__(
        ...
        self._use_features = cfg.objective.type == "multi"
    ...
    def _evaluate(
        ...
        *,
        ...
        if dry_run:
            record.outcome = TrialOutcome.CAD_INFEASIBLE
            self._store_feasibility(trial, 0.0)
            return self._penalty(point, TrialOutcome.CAD_INFEASIBLE)
        ...
        if self._use_features:
            values = [rmse]
            for key in sorted(k for k in cae_result.metrics if k.endswith("_error")):
                values.append(cae_result.metrics[key])
            return tuple(values)
    ...
    def _penalty(self, point: DesignPoint, outcome: TrialOutcome) -> float | tuple[float, ...]:
        ...
        if self._use_features:
            n_features = len(self._cfg.objective.features)
            return tuple([p] * (1 + n_features))
        return p
