...
def calculate_rmse(
    ...
    Args:
        result: 解析結果 (columns: displacement, force)
        target: ターゲットカーブ (columns: displacement, force)
        n_points: 比較点数
...
def calculate_feature_errors(
    ...
    Args:
        result_features: 解析結果の特徴量
        target_features: ターゲットの特徴量
...
def calculate_objectives(
    ...
    for obj_name, obj_val in objectives.items():
        # 誤差名からベース名を抽出（peak_force_error -> peak_force）
