...
class VexisRunner:
    ...
    VEXISへの変更は行わず、ファイルコピーとsubprocess呼び出しで連携
    """
    ...
    def run_analysis(
        ...
        if not step_path.exists():
            logger.error(f"STEPファイルが見つかりません: {step_path}")
            return None
        
        # 1. VEXISのinputディレクトリにSTEPファイルをコピー
        ensure_dir(self.vexis_input)
        target_step = self.vexis_input / f"{job_name}.step"
        copy_file_safe(step_path, target_step)
        logger.info(f"STEPファイルをコピー: {step_path} -> {target_step}")
        ...
        try:
            # CREATE_NO_WINDOW for Windows + CREATE_NEW_PROCESS_GROUP
            creationflags = 0
            if os.name == "nt":
                creationflags = subprocess.CREATE_NEW_PROCESS_GROUP
            
            # Qt GUIを無効化するための環境変数設定
            env = os.environ.copy()
            ...
            proc = subprocess.Popen(
                ...
                stdin=subprocess.DEVNULL,  # VEXISのinput()をEOFで即終了させる
        ...
        # 3. 結果ファイルを確認
        if mesh_only:
            # メッシュのみの場合はVTKファイルを返す
            vtk_path = self.vexis_temp / f"{job_name}.vtk"
            if vtk_path.exists():
                logger.info(f"メッシュ生成完了: {vtk_path}")
                return vtk_path
            else:
                logger.error(f"VTKファイルが見つかりません: {vtk_path}")
                return None
    ...
    def get_graph_path(self, job_name: str) -> Path:
        """結果グラフPNGファイルのパスを取得"""
        return self.vexis_results / f"{job_name}_graph.png"
