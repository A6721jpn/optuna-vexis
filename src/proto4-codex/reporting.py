...
def _plot_optimization_history(study: optuna.study.Study, path: Path) -> Optional[Path]:
    try:
        import matplotlib
        ...
        _write_history_svg(svg_path, xs, ys, best_so_far)
        return svg_path
    ...
    plt.title("Objective Progress Over Iterations")
    plt.xlabel("Trial")
...
def _plot_pareto_front_2d(study: optuna.study.Study, path: Path) -> Optional[Path]:
    ...
    try:
        ...
        svg_path = path.with_suffix(".svg")
        _write_pareto_svg(svg_path, all_points, pareto_points)
        return svg_path
...
def _write_history_svg(path: Path, xs: list[int], ys: list[float], best_so_far: list[float]) -> None:
    ...
    lines = _svg_header(width, height, "Objective Progress Over Iterations")
    lines.append(
...
def _write_pareto_svg(
    ...
    lines = _svg_header(width, height, "Pareto Front (2 objectives)")
    lines.append(
        f'<line x1="{m_left}" y1="{m_top + plot_h}" x2="{m_left + plot_w}" '
        f'y2="{m_top + plot_h}" stroke="#222" stroke-width="1.2"/>'
    )
    ...
    lines.append(
        f'<text x="{width / 2:.1f}" y="{height - 18}" text-anchor="middle" '
        'font-family="sans-serif" font-size="12" fill="#333">Objective 1</text>'
    )
    lines.append(
        f'<text x="18" y="{height / 2:.1f}" transform="rotate(-90 18 {height / 2:.1f})" '
        'text-anchor="middle" font-family="sans-serif" font-size="12" fill="#333">'
...
def _collect_table_columns(
    ...
    objective_cols = [f"objective_{i + 1}" for i in range(len(study.directions))]
...
def generate_markdown_report(
    *,
    ...
    history_path = _plot_optimization_history(study, history_img)

    pareto_img = assets_dir / "pareto_front_2d.png"
    pareto_path = _plot_pareto_front_2d(study, pareto_img)
