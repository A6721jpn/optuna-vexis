...
| --- | --- | --- |
| 実行入口/CLI引数を変える | `scripts/run_v1.py` | `src/v1/runner.py` |
| サンプラー設定・離散化を変更する | `src/v1/search_space.py` | `src/v1/config.py`, `tests/v1/test_physical_discretization.py` |
| CADゲート判定を修正する | `src/v1/cad_gate.py` | `src/v1/objective.py`, `tests/v1/test_search_space_sampler_gate.py` |
| FreeCAD STEP生成の不具合を追う | `src/v1/geometry_adapter.py` | `src/v1/freecad_worker.py`, `src/v1/freecad_engine.py` |
| CAE結果の読み取り/失敗検知を修正する | `src/v1/cae_evaluator.py` | `tests/v1/test_cae_solver_monitor.py` |
| ペナルティ/分岐ロジックを修正する | `src/v1/objective.py` | `src/v1/constraints.py`, `tests/v1/test_constraints_lattice.py` |
| 設定読み込みバリデーション修正 | `src/v1/config.py` | `tests/v1/test_constraints_domain.py`, `tests/v1/test_target_values_optimization.py` |
| 出力形式/レポート修正 | `src/v1/persistence.py` | `src/v1/reporting.py` |
